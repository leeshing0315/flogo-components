...
func main() {
	...
	// conn, err := net.Dial("tcp", "ciot-tcp.cargosmart.ai:8080")
	// conn, err := net.Dial("tcp", "203.99.143.194:8080")
	if err != nil {
		return
	}
	conn.Write([]byte{80, 32, 84, 32, 73, 32, 73, 32, 80, 32, 80, 32, 13, 10})
...
func sendSinglePacket(conn net.Conn) {
	_, err := conn.Write([]byte{ // 80, 32, 84, 32, 73, 32, 73, 32, 80, 32, 80, 32, 13, 10,
		50, 1, 219, 0, 27, 2, 15,
		// 460010604706821
		52, 54, 48, 48, 49, 48, 54, 48, 52, 55, 48, 54, 56, 50, 49,
		0, 8, 72, 83, 49, 56, 48, 54, 48, 53, 147, 146})
	ackBuffer := make([]byte, 7)
	conn.Read(ackBuffer)
	if err != nil {
		println(err.Error())
	...
	_, err = conn.Write([]byte{54, 1, 220, 0, 108, 0, 0, 0, 114, 25, 1, 24, 16, 70, 17, 0, 34, 3, 86, 6, 19, 14, 119, 1, 44, 1, 52, 100, 4, 2, 16, 21, 2, 44, 68, 1, 255, 255, 255, 66, 195, 67, 195, 79, 195, 99, 255, 93, 255, 255, 255, 38, 11, 17, 47, 4, 163, 3, 18, 200, 23, 0, 121, 195, 15, 3, 18, 255, 119, 0, 41, 0, 69, 0, 132, 0, 136, 65,
		1, 33,
		52, 54, 48, 48, 49, 48, 54, 48, 52, 55, 48, 54, 56, 50, 49,
		// C01937
		67, 48, 49, 57, 51, 55,
		// CXRU1338831
		67, 88, 82, 85, 49, 51, 51, 56, 56, 51, 49,
		68,
		141, 126})
	ackBuffer = make([]byte, 7)
	conn.Read(ackBuffer)
	...
	_, err = conn.Write([]byte{52, 0, 3, 0, 0, 0, 0})
